...
class ClientInput(Process):
    ...
    def set_actor(self, actor):
        ...
        self.system.player_input.actor = actor
    ...
    def handle_join(self, message):
        ...
        self.system.camera.focus_on(x, y)
        self.system.player_input.start()

        pos = self.system.component.position(x, y)
        self.world.add(index, pos)
        self.world.update()
    ...
    def handle_world_info(self, message):
        ...
        self.network.login_server(self.network.connection.username, self.network.connection.password)
    ...
    def handle_login(self, message):
        ...
        self.set_actor(index)

        host = self.network.connection.host
        pool = self.network.connection.message_pool

        self.system.entity.add_actor(0, 0, 'soul.png', index)
        pos = self.system.spatial.get(index)
        self.world.add(index, pos)  # TODO get rid of this in favor of an ecs solution
        self.world.update()

        self.network.join_server(self.network.connection.username, self.network.connection.password)
